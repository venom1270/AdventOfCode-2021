...
getLinePoints (x1,y1,x2,y2)
        | x1 == x2 && y1 == y2 = [(x1,y1,0)]
        | x1 < x2 && y1 == y2 = point : getLinePoints (x1+1,y1,x2,y2)
        | x1 > x2 && y1 == y2 = point : getLinePoints (x1-1,y1,x2,y2)
        | y1 < y2 && x1 == x2 = point : getLinePoints (x1,y1+1,x2,y2)
        | y1 > y2 && x1 == x2 = point : getLinePoints (x1,y1-1,x2,y2)
        | x1 < x2 && y1 < y2 = point : getLinePoints (x1+1,y1+1,x2,y2)
        | x1 > x2 && y1 < y2 = point : getLinePoints (x1-1,y1+1,x2,y2)
        | x1 < x2 && y1 > y2 = point : getLinePoints (x1+1,y1-1,x2,y2)
        | x1 > x2 && y1 > y2 = point : getLinePoints (x1-1,y1-1,x2,y2)
...
-- RESULT: 22364
-- Took about 45 mins with compiler optimizations :D
main = do
        let list = []
        ...
        print (getSolution (markPointsOnLines tuples))
