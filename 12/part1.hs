...
findPaths node path adjList paths
    | node == "end" = [node:path]
    | elem node path && isUpper (head node) == False = []
    | otherwise = foldl (\acc x -> (findPaths x (node:path) adjList paths) ++ acc) paths (getAdjacentNodes node adjList)
...
main = do
        ...
        print adjacencyList
        let paths = findPaths "start" [] adjacencyList []
        --print paths
        print $ length paths
