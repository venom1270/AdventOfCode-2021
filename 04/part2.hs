...
bingoGame boards numbersList
        | any (==True) check && length check == 1 = number * sum (filter (>= 0) (head markedBoards))
        | any (==True) check = bingoGame (removeBoards (zip markedBoards check)) numbers
...
removeBoards :: [([Int], Bool)] -> [[Int]]
removeBoards b = map fst (filter (\(_,x) -> not x ) b)
