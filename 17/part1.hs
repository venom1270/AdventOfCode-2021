...
shoot :: (Int, Int, Int, Int) -> Int -> (Int, Int, Int, Int) -> Int
shoot (x,y,xVel,yVel) maxY (bX1,bX2,bY1,bY2)
    | inArea = newMaxY
    | y < bY2 = -9999
    | otherwise = shoot (x+xVel,y+yVel,newVelX,yVel-1) newMaxY (bX1,bX2,bY1,bY2)
...
simulate xVels yVels border = foldl (\acc1 xVel -> acc1++(foldl (\acc2 yVel -> ((shoot (0,0,xVel,yVel) (-9999) border), xVel, yVel):acc2) [] yVels)) [] xVels
...
main = do
        ...
        let xCoords = generateXCoords x
        let yCoords = [0..1000]

        print $ generateXCoords x

        let sim = simulate xCoords yCoords border
        let filteredSim = filter (\(y,_,_) -> y /= -9999) sim
        print sim
        print filteredSim
